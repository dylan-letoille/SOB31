...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player  # removed +1
    return game
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        while not available:  # added colon
            ...
            available = check_space_empty(game, row, column)  # added column
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game)  # uncommented the line
